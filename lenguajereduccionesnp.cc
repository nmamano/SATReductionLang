...
/*
void morirpuroformat(string erroringles,string errorespanyol,string errorcatalan,
         string &muestrain,string &muestraout)
{
  morirpuro("rejected","Format error in the output produced by your program: "+erroringles+"\n\nThe input of your program is:\n\n"+muestrain+"\nThe output of your program is:\n\n"+muestraout,
      "rechazado","Error de formato en la salida producida por tu programa: "+errorespanyol+".\n\nLa entrada de tu programa es:\n\n"+muestrain+"\nLa salida de tu programa es:\n\n"+muestraout,
      "rebutjat","Error de format en la sortida produida pel teu programa: "+errorcatalan+".\n\nLa entrada del teu programa es:\n\n"+muestrain+"\nLa sortida del teu programa es:\n\n"+muestraout);

}

void ejecutaformat(tnodo &nodo,tvalor &valor,int indice,string &muestrain,string &muestraout,string prefijo)
{
  if (nodo.tipo=="struct") {
    if (valor.kind!=4)
      morirpuroformat(prefijo+" must be a struct.",
          prefijo+" debe ser un struct",
          prefijo+" ha de ser un struct.",muestrain,muestraout);
    map<string,tnodo>::iterator itnodo=nodo.m.begin();
    map<string,tvalor>::iterator itvalor=valor.m.begin();
    while (itnodo!=nodo.m.end() or itvalor!=valor.m.end()) {
      if (itvalor==valor.m.end() or (itnodo!=nodo.m.end() and itnodo->first>itvalor->first))
  morirpuroformat("The field "+prefijo+"."+itnodo->first+" must be defined.",
      "El campo "+prefijo+"."+itnodo->first+" tiene que estar definido.",
      "El camp "+prefijo+"."+itnodo->first+" ha d'estar definit."
      muestrain,muestraout);
      if (itnodo==nodo.m.end() or (itvalor!=valor.m.end() and itnodo->first<itvalor->first))
  morirpuroformat("The field "+prefijo+"."+itvalor->first+" must not be defined.",
      "El campo "+prefijo+"."+itvalor->first+" no tiene que estar definido.",
      "El camp "+prefijo+"."+itvalor->first+" no ha d'estar definit."
      muestrain,muestraout);
      ejecutaformat(itnodo->second,itvalor->second,indice,muestrain,muestraout,prefijo+"."+itnodo->first);
    }
  } else if (nodo.tipo=="array") {
----
  ...
  if (nodo.tipo==";") {
    ...
    if (out.kind!=2) {
      memoria-=computausomemoria(out);
      out.kind=2;
      out.v=vector<tvalor> ();
    }
    int lenout=int(out.v.size());
    tvalor v=ejecutaexpresion(nodo.hijo[0],in,valor);
    if (v.kind!=0 and v.kind!=1)
      morir("rejected","Runtime error line "+itos(nodo.linea)+" column "+itos(nodo.columna)+": \"insertsat\" must be applied to strings.",
      "rechazado","Error de ejecucion linea "+itos(nodo.linea)+" columna "+itos(nodo.columna)+": \"insertsat\" se debe aplicar a strings.",
      "rebutjat","Error d'execucio linea "+itos(nodo.linea)+" columna "+itos(nodo.columna)+": \"insertsat\" s'ha d'aplicar a strings.");
    subirastring(v);
    insertarformulasat(v.s,out);
    for (int i=lenout;i<int(out.v.size());i++)
      memoria+=1+computausomemoria(out.v[i]);
    controlmemoria(memoria);
  } else if (nodo.tipo=="push") {
    extraerout(nodo,in,out,valor,memoria,m);
  } else if (nodo.tipo=="if") {
}
  }
...
bool incluido(set<int> s1, set<int> s2)
{
  for (set<int>::iterator it = s1.begin(); it != s1.end(); it++)
    if (int(s2.count(*it)) == 0)
      return false;
  return true;
}
...
int main(int argc,char *argv[])
{
  if (argc!=5)
    ...
    "rebutjat","Error intern: el nombre d'arguments rebuts pel jutge no es correcte.");
  string ficherojp(argv[1]);
  string ficheropreparain(argv[2]);
  string ficheropreparasat(argv[3]);
  string ficheropropuestasolucion(argv[4]);
  string ficheropreparasatout(argv[5]);
  vector<tvalor> jp;
  cout<<"leer jp"<<endl;
  leerjps(ficherojp,jp);
  for (int i=0;i<int(jp.size());i++) {
    cout<<"----"<<endl<<endl;
    cout<<muestratvalor(jp[i])<<endl;
  }
  tnodo nodopreparain,nodopropuestasolucion,nodopreparasatout;
  cout<<"leer preparain"<<endl;
  leerprograma(ficheropreparain,nodopreparain);
  cout<<"leer propuestasolucion"<<endl;
  leerprograma(ficheropropuestasolucion,nodopropuestasolucion);
  cout<<"leer preparasatout"<<endl;
  leerprograma(ficheropreparasatout,nodopreparasatout);
  vector<tvalor> inpropuestasolucion,outpropuestasolucion,outsat;
  cout<<"ejecuta preparain"<<endl;
  ejecuta(nodopreparain,jp,inpropuestasolucion);
  cout<<"ejecuta propuestasolucion"<<endl;
  ejecuta(nodopropuestasolucion,inpropuestasolucion,outpropuestasolucion);
  cout<<"ejecuta preparasatout"<<endl;
  ejecuta(nodopreparasatout,outpropuestasolucion,outsat);
  for (int i=0;i<int(jp.size());i++) {
    cout<<endl<<endl<<endl;
    cout<<"----"<<endl;
    cout<<muestratvalor(inpropuestasolucion[i])<<endl;
